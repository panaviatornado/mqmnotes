...
\end{align*}
Moreover, equality in $Q(P,P)\ge0$ is not possible, since one would require $\xi(x)\xi_2(y)=c\overline{\xi_2(x)}\overline{\xi_1(y)}$, which is not possible since $\xi_1,\xi_2$ are orthogonal. So we obtain $Q(P,P)>0$. %TODO later prove/check equality not possible
